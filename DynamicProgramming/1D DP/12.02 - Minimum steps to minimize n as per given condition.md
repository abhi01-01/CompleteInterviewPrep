### GFG - [Minimum steps to minimize n as per given condition(Loop Version)](https://www.geeksforgeeks.org/problems/minimum-steps-to-minimize-n-as-per-given-condition0618/0)
  ...
  public:
    int solveWithoutMemo(int N){
        ...
        for(int divisor = 2 ; divisor <= 3 ; ++divisor){
            if(N%divisor == 0){
                minSteps = min(minSteps, solveWithoutMemo(N/divisor)) ;
            }
        }
        return (minSteps + 1);
    }
...
class TopDown{
    ...
    int solveWithMemo(int N, vector<int>& dp){
        ...
        for(int divisor = 2 ; divisor <= 3 ; ++divisor){
            if(N%divisor == 0){
                minSteps = min(minSteps, solveWithMemo(N/divisor, dp)) ;
            }
        }
...
class BottomUp{
    ...
    int solveWith1Dtable(int N){
        ...
        for(int i = 2 ; i <= N ; ++i){
            ...
            for(int divisor = 2 ; divisor <= 3 ; ++divisor){
                if(i%divisor == 0){
                    minSteps = min(minSteps, dp[i/divisor]) ;
                }
            }
            dp[i] = (minSteps + 1);            
        }
